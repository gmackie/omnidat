...
describe("create-gmacko-app scaffold", () => {
  ...
  describe("basic scaffolding", () => {
    ...
    it("should scaffold vinext support when requested", async () => {
      ...
      expect(nextPkg.scripts?.["prebuild:vinext"]).toBe(
        "pnpm --dir ../.. --filter @omnidat/nextjs^... build",
      );
    ...
    it("should scaffold a tRPC-backed operator CLI and MCP lane when requested", async () => {
      ...
      expect(rootPackage.scripts?.["trpc:ops"]).toContain("@omnidat/trpc-cli");
      expect(rootPackage.scripts?.["mcp:app"]).toContain("@omnidat/mcp-server");
      expect(operatorCorePackage.name).toBe("@omnidat/operator-core");
      expect(trpcCliPackage.name).toBe("@omnidat/trpc-cli");
      expect(Object.keys(trpcCliPackage.bin ?? {})).toContain("gmacko-ops");
      expect(trpcCliSource).toContain("@omnidat/operator-core");
      expect(mcpServerCoreSource).toContain("@omnidat/operator-core");
    ...
    it("should scaffold without the legacy eslint and prettier stack", async () => {
      ...
      expect(rootPkg.devDependencies?.["@omnidat/prettier-config"]).toBeFalsy();
      ...
      expect(nextPkg.devDependencies?.["@omnidat/eslint-config"]).toBeFalsy();
      expect(nextPkg.devDependencies?.["@omnidat/prettier-config"]).toBeFalsy();
  ...
  it("keeps the CLI E2E workflow aligned with the current template baseline", () => {
    ...
    expect(e2eWorkflow).toContain("pnpm --filter @omnidat/nextjs build");
    expect(e2eWorkflow).toContain("pnpm --filter @omnidat/tanstack-start build");
    expect(e2eWorkflow).toContain("pnpm --filter @omnidat/expo typecheck");
    expect(e2eWorkflow).toContain(
      "pnpm --filter @omnidat/expo exec expo start --dev-client --help",
    );
    expect(e2eWorkflow).toContain(
      "pnpm --filter @omnidat/expo exec expo config --json",
    );
    expect(e2eWorkflow).toContain(
      "pnpm --filter @omnidat/nextjs deploy:cloudflare:staging",
    );
  ...
  describe("integration options", () => {
    ...
    it("should prune unused packages when --prune is passed", async () => {
      ...
      expect(nextProviders).not.toContain("@omnidat/analytics/web");
      expect(nextErrorPage).not.toContain("@omnidat/monitoring/web");
      expect(nextGlobalError).not.toContain("@omnidat/monitoring/web");
      expect(nextErrorBoundary).not.toContain("@omnidat/monitoring/web");
